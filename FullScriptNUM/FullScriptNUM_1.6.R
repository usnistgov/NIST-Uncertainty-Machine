## AMP 2021-Oct-13 -- START
## Please confirm that the version is set correctly
## Version
cat("NIST Validation & Verification R Script Version 1.6 \r\n")
## AMP 2021-Oct-13 -- END
...
distrib = function(nbVar,nbReal,varNames,expression,type,parameters,symmetrical,correlation,outputFile,copulaType=NULL,copulaDf=NULL,pretreatment=NA)
{
    ...
    ## AMP 2021-Oct-13 -- START
    if (nbReal < 10000) {
        errorStop(paste0("The number of realizations should be greater than ",
                         "9999\n"), errorFile) }
    ## AMP COMMENT: There may be cases, like the measurement of the
    ## age of the basalt from the Sea of Tranquility, that involve
    ## substantial calculations, and for which this requirement about
    ## the size of the Monte Carlo sample may be excessive. Therefore,
    ## I'm proposing that we reduce this number from the original
    ## 100000 to 10000. Since the code invokes "significator" (below),
    ## the user should be able to conclude whether the specified
    ## sample size is too small for his purposes. However, we don't
    ## seem to get this information in all cases. Also, but possibly
    ## not now, we need to rethink the use of "setTimeLimit" (below). 
    ## AMP 2021-Oct-13 -- END
    ...
    ## AMP 2021-Oct-13 -- START
    ## AMP COMMENT: I do not remember whether the construction of an
    ## environment involving only R functions deemed "safe," was
    ## motivated by NIST IT requirements, and also wonder what
    ## "safety" this actually buys us.
    ## ISSUE: The code for the Sea of Tranquility example involves
    ## "sum" and "optim" -- please check whether I've added them in
    ## the best way. It would be good to have ":" as well, to create a
    ## numerical sequence succinctly
    ...
    for (f in safe_f) {	varEnv[[f]] <- get(f, "package:base") }
	...
	if(outputFile!="no-output")
	{
		...
		par(mar=c(4.5, 4.5, 0.5, 0.75))
                ...
                ## AMP 2021-Oct-09 -- START
                ## ISSUE: Ensure that neither density gets clipped
                ## ISSUE: Lack of curly brackets in if {} else {} clause
                ## hinders line-by-line manual execution
		if (parallelL2) {
                    mcparallel(
                    {
...
drawDistrib = function(type,parameters,varNames,varEnv,errorFile,ii)
{
	...
	if(type[ii]==21)  ## Beta (Mean, StdDev, Left, Right)
	{
        ...
        ## AMP 2021-Oct-08 -- START
        ## ISSUE: meanX and sdX were incorrect because they did not take
        ## the rescale and shift into account
